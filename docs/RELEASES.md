...
The core workflow creates the authoritative GitHub Release. For the controlled-pilot scope, users install from the release page or inspect and run the matching installer described in [Installation](INSTALLATION.md). Public 1.x distribution remains the scope of [issue #19](https://github.com/joicepassos/harness-forge/issues/19): before claiming that release scope is complete, maintain the Homebrew tap, Scoop manifest/bucket, and checksum-validating `@harnessforge/cli` installer described there. They are deferred from the controlled pilot, not excluded from the public distribution plan.
