...
Do not publish a package version unless the GitHub Release with the same version already exists. The release workflow sets the npm package version from the protected Git tag in its temporary workspace. Starting with v1.1.1, the guided `init` flow is included in the release binary.
