...
Do not publish a package version unless the GitHub Release with the same version already exists. Restore `package.json` after the bootstrap publish; the release workflow sets the npm package version from the protected Git tag in its temporary workspace.
